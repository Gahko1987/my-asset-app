...
with st.expander("▼ 基本設定（ここをタップして変更）", expanded=True):
    ...
    with col_b1:
        # 年齢（変更時に再計算キーを設定）
        current_age = st.number_input("現在の年齢", 20, 80, 39, key="input_current_age")
        current_assets = st.number_input("現在の資産 (万円)", 0, 50000, 2300)
...
if st.button("シミュレーションを実行する", type="primary"):
    ...
    try:
        ...
        st.subheader(f"シミュレーション結果")
        res_col1, res_col2, res_col3, res_col4 = st.columns(4)
        res_col1.metric("95歳生存率", f"{100 - ruin_prob:.1f}%")
