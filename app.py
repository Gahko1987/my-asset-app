初心者の方でも迷わず使えるよう、「これをそのまま貼り付ければOK」 という状態にまとめた全文コードをご用意しました。

ご希望通り、最初から「5歳のお子様」と「2歳のお子様」が入力された状態 になっています。

手順
GitHubの app.py を開き、中身をすべて消して真っ白にします。

下のコードをコピーして貼り付けます。

保存（Commit）したら、アプリ画面の右上メニューから 「Clear cache」→「Rerun」 してください。
